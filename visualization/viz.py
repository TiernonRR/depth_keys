...
'''
    Example for keypoint overlay

def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(
        description="Process video with 3D keypoint overlays in batches"
    )
    parser.add_argument('dir', help='Path to session directory')
    parser.add_argument('vnum', help='Version number for keypoint files')
    parser.add_argument('--nframes', type=int, help='Number of frames to process (optional)', default=None)
    parser.add_argument('--frame_start', type=int, help='frame range start (optional)', default=None)
    parser.add_argument('--frame_end', type=int, help='frame range end (optional)', default=None)

    parser.add_argument('--batch-size', type=int, help='Batch size for processing frames', default=1000)
    parser.add_argument('--raw', action="store_true", help='Flag to determine if raw data should be used')
    parser.add_argument('--cam_by_conf', action="store_true", help='Flag to determine if camera name determine by conf')

    
    args = parser.parse_args()
    
    processor = KeypointVideoProcessor(
        args.dir, 
        args.vnum, 
